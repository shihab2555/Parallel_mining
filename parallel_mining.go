...
func main() {
	t := "Null"
	genesisBlock := Block{}
	genesisBlock = Block{0, t, 0, calculateHash(genesisBlock), "", difficulty, "", 10000}
...
func calculateHash(block Block) string {
	record := strconv.Itoa(block.Index) + strconv.Itoa(block.Transaction_record) + block.PrevHash + block.Nonce + block.Timestamp 
	h := sha256.New()
	h.Write([]byte(record))
...
func generateBlock(oldBlock Block, TR int, NON int) Block {
	...
	var n uint64
	n = uint64(NON)
	n = (n*10000000000000000)-9999999999999999 
	//t := "Time"
	//fmt.Println("I AM ",n)
	...
	for i = n; ; i++ {
		...
		if (i%60000000 == 0){
			...
			fmt.Println("************************************************************")
			fmt.Println("Transaction time=", time.Since(start))
			fmt.Println("************************************************************")
			t:= time.Since(start)
